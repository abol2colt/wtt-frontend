...
export class GitlabSyncService {
  ...
  private normalizeIntegrationError(
    error: unknown,
    ...
    if (error instanceof HttpErrorResponse) {
      if (error.status === 0) {
        return new Error(
          "Integration proxy در دسترس نیست. اول wtt-proxy را اجرا کن.",
        );
      }

      const backendMessage =
        error.error?.error || error.error?.message || error.error?.description;
        ...
        catchError((error) =>
          throwError(() =>
            this.normalizeIntegrationError(
              error,
              "خطا در تولید گزارش از شواهد Git.",
            ),
          ),
        ),
      );
